        ...
        if quote == 'BYN' or base == 'BYN':
            if quote == 'BYN':
                if base not in currencie.keys():
                    ...
                    total_base = round((currencie[base][0] / currencie[base][2]) * amount, 2)
            else:
                if quote not in currencie.keys():
                    raise APIException(f'Я не знаю волюту "{quote}", '
                                       f'Попробуй ешё раз\n\nПример ввода: 10 USD BYN')
                else:
                    total_base = round((currencie[quote][2] / currencie[quote][0]) * amount, 2)
        else:
            if quote not in currencie.keys():
                raise APIException(f'Я не знаю волюту "{quote}", '
                                   f'Попробуй ешё раз\n\nПример ввода: 10 USD BYN')
            ...
            total_base = round(
                ((currencie[quote][2] / currencie[quote][0]) * amount) / (currencie[base][2] / currencie[base][0]),
                2)
        return total_base
